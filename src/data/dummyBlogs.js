/**
 * Dummy Blog Data
 * Provides rich sample content for blog pages
 */
...
export const getDummyBlogById = (id) => {
  return dummyBlogs.find(blog => blog.id === id);
};

// Get related blogs by category (excluding current blog)
export const getRelatedBlogs = (categoryId, currentBlogId, limit = 3) => {
  const related = dummyBlogs.filter(blog => 
    blog.category === categoryId && blog.id !== currentBlogId
  );
  
  // If not enough related articles in the same category, get most recent ones
  if (related.length < limit) {
    const otherBlogs = dummyBlogs
      .filter(blog => blog.id !== currentBlogId && blog.category !== categoryId)
      .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
    
    return [...related, ...otherBlogs].slice(0, limit);
  }
...
export const getBlogsByCategory = (categoryId) => {
  return categoryId 
    ? dummyBlogs.filter(blog => blog.category === categoryId)
    : dummyBlogs;
};
